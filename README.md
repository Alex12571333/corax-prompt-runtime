...
files below `<data_root>/<root>` and are never allowed to overwrite operator
edits. Legacy `system.md` and `safety.md` are migrated first when supplied.
Identity defaults are `<data_root>/identity/USER.md` and `MEMORY.md`.
