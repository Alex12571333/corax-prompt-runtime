...
class PromptRuntime(RuntimeService):
    ...
    def _object_namespace_layer(self, value: Any) -> PromptLayer | None:
        ...
        content = (
            ...
            "in one search. When a search activates methods, the host ends "
            "that `object_run`; continue with the appended methods in the next "
            "call. If it activates nothing, another search may run here. "
    ...
    def _object_runtime_layer(
        ...
        content = (
            ...
            "only the ephemeral runner. When a listed `self.files.*` method is "
            "present, its paths are interpreted by the host, not inside the "
            "runner: use Corax "
